  ...
  return (
    <>
      ...
      <div
        ...
        className={`fixed inset-0 h-[100dvh] bg-surface-container-lowest z-40 flex flex-col transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
      ...
      >
          ...
          {NAV_ITEMS.map(({ href, label }) => {
            ...
            return (
              <Link
                ...
                href={href}
                onClick={toggle}
                className={`font-playfair text-[clamp(1.35rem,4vw,1.9rem)] leading-snug py-1 transition-colors duration-300 self-start ${
                  active
        ...
        {/* Footer: compact — social + profile + admin */}
        <div className="shrink-0 border-t border-outline-variant/25 px-margin-mobile md:px-margin-desktop py-3 flex flex-col gap-2.5">
          {/* Social icons */}
          ...
          {/* Founder profile + admin */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full overflow-hidden border border-outline-variant bg-surface-container flex-shrink-0">
                <img src="/camille-portrait.webp" alt="Camille" className="w-full h-full object-cover object-[center_20%]" />
              </div>
              <div>
                <p className="font-playfair text-sm text-on-surface leading-tight">Camille</p>
                <p className="text-[10px] text-on-surface-variant" style={{ fontFamily: 'var(--font-hanken)', fontWeight: 300 }}>
            ...
            <Link
              href="/admin"
              onClick={() => setIsMenuOpen(false)}
              className="text-[10px] uppercase tracking-[0.2em] text-on-surface-variant/40 hover:text-on-surface-variant transition-colors duration-300"
              style={{ fontFamily: 'var(--font-hanken)' }}
            >
              Admin
            </Link>
